...
class FormManager {
  ...
  discoverFormFields(formConfig) {
    const inputs = formConfig.element.querySelectorAll('input, textarea, select');
  ...
  parseValidationRules(input) {
    ...
    if (input.hasAttribute('pattern')) {
      ...
      const message = input.getAttribute('data-pattern-message') || '格式不正確';
  ...
  bindGlobalEvents() {
    ...
    document.addEventListener('keydown', (event) => {
      if (event.key === 'Enter' && event.target.matches('input:not([type="textarea"])')) {
  ...
  validateField(fieldConfig, formConfig) {
    ...
    this.executeHooks(formConfig.hooks.beforeValidate, { field: fieldConfig, form: formConfig });
  ...
  async handleFormSubmit(event, formConfig) {
    ...
    try {
      ...
      await this.executeHooks(formConfig.hooks.beforeSubmit, { form: formConfig, event });
  ...
  showAlert(title, message, type = 'info') {
    // 發送自訂事件
    document.dispatchEvent(new CustomEvent('form:alert', {
